...
#   VGI_TRANSLATE_WORKER  worker LOCATION the .test files ATTACH (a stdio command
#                         such as `uv run translate_worker.py`, or an http:// URL)
# Optional:
#   STAGE                 scratch dir for the preprocessed test tree (default: mktemp)
#
# Note: the real-translation cases need the Argos en->es language package
# installed into the user data home (`scripts/ensure_argos_pair.py en es`,
# done by the workflow before this script runs).
set -euo pipefail

: "${HAYBARN_UNITTEST:?path to the haybarn-unittest binary}"
: "${VGI_TRANSLATE_WORKER:?worker LOCATION (stdio command or http:// URL)}"
...
echo "Running suite (worker: $VGI_TRANSLATE_WORKER) ..."
