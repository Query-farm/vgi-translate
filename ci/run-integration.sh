...
#
# Run this repo's sqllogictest suite (test/sql/*.test) against the vgi-translate
...
# ATTACH (`${VGI_TRANSLATE_WORKER}`):
#
#   subprocess : a bare stdio command (`uv run translate_worker.py`) — the extension
...
#                        `uv run --python 3.13 <repo>/translate_worker.py`.
# Optional:
...
WORKER_CMD="${WORKER_CMD:-uv run --python 3.13 $REPO/translate_worker.py}"
...
# ---------------------------------------------------------------------------
# Per-transport: resolve VGI_TRANSLATE_WORKER (the LOCATION) and, for the out-of-band
...
case "$TRANSPORT" in
  subprocess)
    export VGI_TRANSLATE_WORKER="$WORKER_CMD"
    ;;
  ...
  http)
    ...
    PORT_FILE="$(mktemp -u "${TMPDIR:-/tmp}/translate-port.XXXXXX")"
    LOG_FILE="${TMPDIR:-/tmp}/translate-http-server.log"
    ...
    export VGI_TRANSLATE_WORKER="http://127.0.0.1:$PORT"
    ;;

  unix)
    # Boot the worker bound to an AF_UNIX socket; poll for the socket file.
    SOCK="${TMPDIR:-/tmp}/translate-$$.sock"
    rm -f "$SOCK"
    LOG_FILE="${TMPDIR:-/tmp}/translate-unix-server.log"
    ...
    export VGI_TRANSLATE_WORKER="unix://$SOCK"
    ;;
...
echo "Running suite (transport: $TRANSPORT, worker: $VGI_TRANSLATE_WORKER) ..."
OUT_FILE="$(mktemp)"
set +e
