# CI: the vgi-translate worker integration suite

[`.github/workflows/ci.yml`](../.github/workflows/ci.yml) runs the unit tests
and this repo's sqllogictest suite (`test/sql/*.test`) against the vgi-translate
...
1. **Install the worker** — `uv sync --frozen` into a venv. `translate_worker.py`
   is a self-contained PEP 723 stdio worker the extension can spawn via
   `uv run translate_worker.py`. The real-translation cases additionally need
   the Argos **en→es** language package, installed into the user data home by
   `scripts/ensure_argos_pair.py en es` before the suite runs (the `detect_lang`
   case is offline and always runs).
...
3. **Preprocess** — the standalone runner links none of the extensions the
   ...
   the `requires 'target'` haybarn directive, and everything else pass through
   untouched.
4. **Run** — [`run-integration.sh`](run-integration.sh) stages the preprocessed
   tree, points `VGI_TRANSLATE_WORKER` at `uv run translate_worker.py`, warms the
...
uv run python scripts/ensure_argos_pair.py en es   # install the Argos en->es package
...
as a uv tool, ensures the en→es package, and points the worker at
`uv run --python 3.13 translate_worker.py`.
